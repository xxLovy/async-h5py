...
def create(ObjectID loc not None, object name, TypeID tid not None,
    ...
    if cname != NULL:
        dsid = H5Dcreate_async(loc.id, cname, tid.id, space.id, pdefault(lcpl), pdefault(dcpl), pdefault(dapl), es_id)
        print('Using h5py with async HDF5 to create a dataset')
    else:
...
@with_phil
def open(ObjectID loc not None, char* name, PropID dapl=None, int es_id=0):
